...
for(const [table,column,values] of [
  ["timeline_events","source",["demo_seed","Personal health record"]],
  ["medications","prescriber",["Dr. Elena Carter (Demo)","Dr. Elena Carter"]],
  ["appointments","notes",["Demo appointment for promotional account.","Bring the latest blood-pressure readings and medication list.","Annual vision and retinal screening.","Routine six-month cleaning."]],
  ["memories","mood",["demo-joyful","demo-calm","demo-proud","Joyful","Calm","Proud"]],
  ["medical_expenses","notes",["demo_seed","Routine dental cleaning","Monthly prescriptions","Routine blood tests"]],
...
ok(await db.from("check_ins").delete().eq("user_id",userId).in("transcript",["Energy was good after a 30-minute morning walk.","Slept well and remembered all scheduled medication.","Mild knee stiffness after gardening; noted for the next visit."]),"clean current check-ins");
ok(await db.from("doctor_questions").delete().eq("user_id",userId).like("question","Demo:%"),"clean questions");
ok(await db.from("doctor_questions").delete().eq("user_id",userId).in("question",["Should we review the current blood-pressure trend?","Is the next routine blood test due this year?"]),"clean current questions");
ok(await db.from("user_notifications").delete().eq("user_id",userId).ilike("body","%demo%"),"clean notifications");
ok(await db.from("user_notifications").delete().eq("user_id",userId).in("title",["Medication refill coming up","Family check-in added"]),"clean current notifications");
...
ok(await db.from("family_history").delete().eq("user_id",userId).in("relative_name",["Margaret Johnson","Samuel Johnson"]),"clean current family history");
...
ok(await db.from("medications").insert([
  {user_id:userId,name:"Amlodipine",dosage:"5 mg",schedule:"Once each morning",reason:"Blood pressure",prescriber:"Dr. Elena Carter",refill_date:"2026-09-08",expiration_date:"2027-04-30",remaining_pills:24,active:true},
  {user_id:userId,name:"Metformin",dosage:"500 mg",schedule:"Twice daily with meals",reason:"Blood sugar management",prescriber:"Dr. Elena Carter",refill_date:"2026-08-29",expiration_date:"2027-02-28",remaining_pills:38,active:true},
  {user_id:userId,name:"Vitamin D3",dosage:"1,000 IU",schedule:"Once daily",reason:"Vitamin D support",prescriber:"Dr. Elena Carter",refill_date:"2026-10-15",expiration_date:"2027-08-31",remaining_pills:61,active:true},
]),"medications");
ok(await db.from("appointments").insert([
  {user_id:userId,title:"Routine primary-care review",provider:"Dr. Elena Carter",starts_at:"2026-09-04T07:30:00Z",location:"Riverside Family Clinic",notes:"Bring the latest blood-pressure readings and medication list.",follow_up_due:"2027-03-04"},
  {user_id:userId,title:"Annual eye examination",provider:"Dr. Noah Kim",starts_at:"2026-10-18T09:00:00Z",location:"Clearview Eye Centre",notes:"Annual vision and retinal screening."},
  {user_id:userId,title:"Dental cleaning",provider:"Dr. Priya Shah",starts_at:"2026-07-12T08:00:00Z",location:"Harbour Dental",notes:"Routine six-month cleaning.",completed_at:"2026-07-12T08:45:00Z"},
]),"appointments");
ok(await db.from("check_ins").insert([
  {user_id:userId,transcript:"Energy was good after a 30-minute morning walk.",recorded_at:"2026-08-14T06:45:00Z"},
  {user_id:userId,transcript:"Slept well and remembered all scheduled medication.",recorded_at:"2026-08-12T07:10:00Z"},
  {user_id:userId,transcript:"Mild knee stiffness after gardening; noted for the next visit.",recorded_at:"2026-08-09T17:30:00Z"},
]),"check-ins");
const events=[
...
].map(([category,title,description,occurred_at])=>({user_id:userId,category,title,description,occurred_at,source:"Personal health record"}));
ok(await db.from("timeline_events").insert(events),"timeline");
ok(await db.from("memories").insert([
  {user_id:userId,memory_date:"2026-07-24",caption:"Ava graduated today. Everyone made it, and we took a photograph together after the ceremony.",people:["Maya","Daniel","Ava"],mood:"Proud"},
  {user_id:userId,memory_date:"2026-06-08",caption:"Morning walk through the botanical garden with Maya.",people:["Maya"],mood:"Calm"},
  {user_id:userId,memory_date:"2025-12-25",caption:"Christmas lunch at Daniel’s house, with the old family recipes on the table.",people:["Daniel","Maya","Ava"],mood:"Joyful"},
]),"memories");
...
for(const [name,folder,date,type,summary,provider,facility,facts] of documents){
  ...
  ok(await db.from("document_extractions").insert({user_id:userId,document_id:doc.id,model:"gemini-3.5-flash",prompt_version:"health-document-review-v2",output:{documentType:type,summary,documentDate:date,provider,facility,facts,followUpItems:[],timelineDraft:{title:name.replace(/\.pdf$/,""),description:summary,occurredOn:date},medications:[],warnings:[]},confirmed_items:[],confirmed_at:stamp}),`review ${name}`);
  ok(await db.from("ai_requests").insert({user_id:userId,feature:"document_extraction",status:"SUCCEEDED",model:"gemini-3.5-flash",completed_at:stamp}),`AI record ${name}`);
}
...
ok(await db.from("emergency_profiles").upsert({user_id:userId,blood_type:"O+",allergies:["Penicillin"],conditions:["Hypertension","Type 2 diabetes"],emergency_contact:"Maya Johnson · +1 555 010 2040",insurance:"Horizon Health · HH-48291",doctor_contact:"Dr. Elena Carter · Riverside Family Clinic",is_enabled:true,updated_at:stamp},{onConflict:"user_id"}),"emergency card");
ok(await db.from("doctor_questions").insert([{user_id:userId,question:"Should we review the current blood-pressure trend?"},{user_id:userId,question:"Is the next routine blood test due this year?"}]),"questions");
...
ok(await db.from("medical_expenses").insert([
  {user_id:userId,expense_date:"2026-07-12",category:"Dental",provider:"Harbour Dental",amount:120,currency:"USD",insurance_paid:80,notes:"Routine dental cleaning"},
  {user_id:userId,expense_date:"2026-06-03",category:"Medication",provider:"Greenleaf Pharmacy",amount:46.5,currency:"USD",insurance_paid:20,notes:"Monthly prescriptions"},
  {user_id:userId,expense_date:"2025-11-18",category:"Laboratory",provider:"Riverside Family Clinic",amount:95,currency:"USD",insurance_paid:70,notes:"Routine blood tests"},
]),"expenses");
ok(await db.from("family_history").insert([
  {user_id:userId,relative_name:"Margaret Johnson",relationship:"Mother",conditions:["Type 2 diabetes"]},
  {user_id:userId,relative_name:"Samuel Johnson",relationship:"Father",conditions:["Hypertension","Heart disease"]},
]),"family history");
for(const [month,weight,mood] of [["2026-06-01",76.4,"Active"],["2026-07-01",75.8,"Positive"],["2026-08-01",75.5,"Calm"]])ok(await db.from("health_capsules").upsert({user_id:userId,month,snapshot:{profile:{weight_kg:weight,conditions:["Hypertension","Type 2 diabetes"]},medications:["Amlodipine","Metformin","Vitamin D3"],memories:[{mood}],checkIns:[{summary:"Monthly check-in completed"}],appointments:[]},locked_at:stamp},{onConflict:"user_id,month"}),`capsule ${month}`);
ok(await db.from("subscriptions").upsert({user_id:userId,provider:"manual",plan:"FAMILY",status:"ACTIVE",provider_reference:`seed-${userId}`,current_period_end:"2030-01-01T00:00:00Z",updated_at:stamp},{onConflict:"user_id"}),"family plan");
ok(await db.from("user_notifications").insert([{user_id:userId,title:"Medication refill coming up",body:"Metformin has a refill date in two weeks.",href:"/medications"},{user_id:userId,title:"Family check-in added",body:"Maya added a new family journal entry.",href:"/family"}]),"notifications");
