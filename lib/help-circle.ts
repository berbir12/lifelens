...
export const helpTaskSchema=z.object({title:z.string().trim().min(1).max(120),details:z.string().trim().max(500).optional(),dueAt:z.string().max(40).optional().or(z.literal(""))}).refine(value=>!value.dueAt||!Number.isNaN(new Date(value.dueAt).getTime()),{message:"Invalid date"});
