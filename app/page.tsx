...
export default function Landing(){return <main className="min-h-screen bg-[#f4f2ec] font-sans text-[#182019] selection:bg-[#d5dccf]">
  ...
  <section id="pricing" className="mx-auto max-w-[1440px] border-x border-b border-[#182019]/20">
    <div className="grid border-b border-[#182019]/20 lg:grid-cols-2">
      ...
      <div className="flex items-end p-6 md:p-10"><div><p className="max-w-lg text-base leading-7 text-black/55">Every plan includes private storage, manual records, timeline search, and structured data export. Paid plans increase document and family-circle capacity.</p><p className="mt-5 text-xs text-black/40">Prices shown in USD. Paid purchases provide 30 days of access and do not renew automatically.</p></div></div>
    </div>
    <div className="grid lg:grid-cols-3">
      ...
      <Price name="Plus" price="$8" cadence="for 30 days" description="For an active, long-term record." features={["250 stored documents","Everything in Personal","Health streaks and capsules","2 family members","Printable doctor visit summary"]} featured/>
      <Price name="Family" price="$14" cadence="for 30 days" description="For ongoing support from a care circle." features={["500 stored documents","Everything in Plus","Up to 5 family members","Permissioned family check-ins","Emergency card and health passport"]}/>
    </div>
    <div className="flex flex-col justify-between gap-4 border-t border-[#182019]/20 p-6 text-sm md:flex-row md:items-center md:p-10"><div><b>Simple, non-recurring access.</b><p className="mt-1 text-xs text-black/45">Buy another 30-day period whenever you need it.</p></div><p className="max-w-lg text-xs leading-5 text-black/45">Document limits count successfully stored files. Existing files remain available after paid access ends; new uploads pause above the Personal limit.</p></div>
  </section>
  ...
  <footer className="border-t border-white/10 bg-[#1d2d23] text-white"><div className="mx-auto flex max-w-[1440px] flex-col justify-between gap-8 px-5 py-10 md:flex-row md:px-10"><div><b>LifeLens</b><p className="mt-2 text-xs text-white/40">A personal health record.</p></div><div className="max-w-md text-xs leading-5 text-white/40">LifeLens organizes information provided by its users. It does not diagnose, prescribe, or replace a healthcare professional.</div><div className="flex gap-5 text-sm"><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/login" className="flex items-center gap-2">Sign in <ArrowUpRight size={14}/></Link></div></div></footer>
</main>}

function Price({name,price,cadence,description,features,featured=false}:{name:string;price:string;cadence:string;description:string;features:string[];featured?:boolean}){return <article className={`flex min-h-[540px] flex-col border-b border-[#182019]/20 p-6 last:border-b-0 md:p-10 lg:border-b-0 lg:border-r lg:last:border-r-0 ${featured?"bg-[#e5e8df]":"bg-[#f4f2ec]"}`}><div className="flex items-start justify-between"><h3 className="text-lg font-semibold">{name}</h3>{featured&&<span className="border border-[#182019]/25 px-2 py-1 text-[10px] font-semibold uppercase tracking-[.12em]">Most useful</span>}</div><p className="mt-5 max-w-xs text-sm leading-6 text-black/50">{description}</p><div className="mt-12 border-b border-[#182019]/20 pb-8"><span className="text-5xl font-medium tracking-[-.06em]">{price}</span><span className="ml-2 text-xs text-black/45">{cadence}</span></div><ul className="mt-7 space-y-4">{features.map(feature=><li key={feature} className="flex gap-3 text-sm"><Check className="mt-0.5 shrink-0" size={14}/>{feature}</li>)}</ul><Link href="/login" className={`mt-auto flex h-11 items-center justify-between border px-4 text-sm font-semibold ${featured?"border-[#182019] bg-[#182019] text-white":"border-[#182019]/30 hover:border-[#182019]"}`}>Choose {name}<ArrowUpRight size={14}/></Link></article>}
