...
export async function POST(req:NextRequest,{params}:{params:Promise<{slug:string}>}){
  ...
  const {data,error}=await supabase.rpc("public_add_help_task",{board_slug:slug,task_title:parsed.data.title,task_details:parsed.data.details||null,task_due_at:parsed.data.dueAt?new Date(parsed.data.dueAt).toISOString():null,helper_name:parsed.data.name});
